...
export function updateMap(){
  ...
  items.forEach((o,i) => {
    ...
    const marker = L.circleMarker(coords, {
      ...
      fillColor: statusColors[o.status] || '#666',
      fillOpacity: .85,
      weight: 3, radius: 10
    }).addTo(map);
    ...
    const lm = L.marker(coords, { icon: label }).addTo(map);
    mapMarkers.push(marker, lm);
  });

  const hub = L.marker(TRANSPORT_HUB.coords).addTo(map);
  hub.bindPopup(`<strong>${TRANSPORT_HUB.name}</strong><br>${TRANSPORT_HUB.address}`);
  mapMarkers.push(hub);

  ['ALMERE','VENLO','ZWIJNDRECHT'].forEach(LN => {
    if(!loc || loc === LN){
      const coords = getCoordinatesForLocation(LN);
      if(coords){
        const m = L.marker(coords).addTo(map);
        m.bindPopup(`<strong>Motrac ${LN}</strong>`);
        mapMarkers.push(m);
      }
    }
  });

  // simplified: routes are not drawn as polylines in this demo version
}
